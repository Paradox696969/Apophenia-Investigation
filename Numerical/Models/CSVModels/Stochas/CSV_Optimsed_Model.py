...
features, labels = loadDataset("Numerical/Datasets/CSV/Stochas.csv")
...
def save(name):
    result_str = f"{name},"
    test_features, test_labels = loadDataset("Numerical/Datasets/CSV/LowTest.csv", False)
    result = model.evaluate(test_features, test_labels)
    result_str += f"{round(result, 6)},"
    test_features, test_labels = loadDataset("Numerical/Datasets/CSV/StochasTest.csv", False)
    result = model.evaluate(test_features, test_labels)
    result_str += f"{round(result, 6)}"
...
model.save("Numerical/Models/Saved/Optimised-StochasCSV_Model.h5")