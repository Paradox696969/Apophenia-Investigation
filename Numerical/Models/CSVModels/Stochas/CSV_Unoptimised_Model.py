...
features, labels = loadDataset("Numerical/Datasets/CSV/Stochas.csv", True)
...
model.fit(features, labels, epochs=25, validation_split=0.2, batch_size=128)

def save(name):
    result_str = f"{name},"
    test_features, test_labels = loadDataset("Numerical/Datasets/CSV/LowTest.csv", True)
    result = model.evaluate(test_features, test_labels)
    result_str += f"{round(result, 6)},"
    test_features, test_labels = loadDataset("Numerical/Datasets/CSV/StochasTest.csv", True)
    result = model.evaluate(test_features, test_labels)
    result_str += f"{round(result, 6)}"
...
model.save("Numerical/Models/Saved/Unoptimised-StochasCSV_Model.h5")